...
describe('resolveTarget', () => {
  it('default is stg with stg db path', () => {
    const r = resolveTarget(undefined, undefined, empty);
  ...
  it('CLI target beats env', () => {
    ...
    const r = resolveTarget('prod', undefined, env({ KIJUKU_TARGET: 'stg' }));
  ...
  it('env beats default', () => {
    const r = resolveTarget(undefined, undefined, env({ KIJUKU_TARGET: 'prod' }));
    expect(r.target).toBe('prod');
    expect(r.readonly).toBe(true);
  });

  it('invalid env falls back to stg', () => {
    const r = resolveTarget(undefined, undefined, env({ KIJUKU_TARGET: 'bogus' }));
    expect(r.target).toBe('stg');
  });

  it('CLI db beats all (readonly still derived from target)', () => {
    // --db が target 別 env/デフォルトに勝つ（target=prod でも --db を尊重）
    const r = resolveTarget('prod', '/custom/path.db', env({ KIJUKU_DB_PATH: '/env/prod.db' }));
  ...
  it('per-target env db path', () => {
    // prod + KIJUKU_DB_PATH
    const prod = resolveTarget('prod', undefined, env({ KIJUKU_DB_PATH: '/env/prod.db' }));
    expect(prod.dbPath).toBe('/env/prod.db');
    // stg + KIJUKU_STG_DB_PATH
    const stg = resolveTarget('stg', undefined, env({ KIJUKU_STG_DB_PATH: '/env/stg.db' }));
    expect(stg.dbPath).toBe('/env/stg.db');
  });

  it('stg ignores prod env var', () => {
    // target=stg のとき KIJUKU_DB_PATH(prod用) は無視して stg デフォルト
    const r = resolveTarget('stg', undefined, env({ KIJUKU_DB_PATH: '/env/prod.db' }));
    expect(r.dbPath).toBe('kijuku.stg.db');
  });
});
