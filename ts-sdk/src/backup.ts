...
export class BackupManager {
  ...
  constructor(private db: Database.Database, options: BackupOptions) {
  ...
  async backup(): Promise<string> {
    ...
    const backupFileName = `kijuku-backup-${timestamp}.db`;
  ...
  listBackups(): Array<{ name: string; path: string; createdAt: Date }> {
    ...
    return fs
      ...
      .filter((file) => file.startsWith('kijuku-backup-') && file.endsWith('.db'))
      .map((file) => ({
        name: file,
