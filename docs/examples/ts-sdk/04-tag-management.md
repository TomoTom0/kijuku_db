...
> **パフォーマンス注意**: この実装はタグの数だけクエリを発行するため、タグ数が多い場合はパフォーマンスが低下します（N+1クエリ問題）。上記の`getTagUsageStats()`を使用してください。
...
> **パフォーマンス注意**: この実装はタグの数だけクエリを発行するため、タグ数が多い場合はパフォーマンスが低下します（N+1クエリ問題）。上記の`findUnusedTags()`を使用してください。
