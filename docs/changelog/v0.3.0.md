...
### CLI の Rust 単一化・Node.js 版 kijuku-cli 廃止（TASK-63,64,65,66,68）

CLI 実装を Rust に一本化し、Node.js 版 CLI（`ts-sdk/src/cli.ts` + `package.json` の `kijuku-cli` bin）を廃止した。旧 ADR「TypeScript SDK 優先」を「Rust SDK 優先」へ更新（`docs/design/decisions.md`・TASK-66）。

- **廃止（TASK-65）**: `ts-sdk/src/cli.ts`（668行）と `kijuku-cli` bin エントリを削除。TS SDK のライブラリ機能は維持（CLI としての配布は終了）。`createDatabase`/`parseDbPath` ファクトリは SDK 側へ移行
- **SSH remote 移植（TASK-64）**: Rust CLI に `--db host:path` でリモート DB 接続を追加。不足していた `RemoteKijukuDB` メソッドラッパ 8種（sync/observe/promote/listPreStashes/setBackupLabel/setBackupNote/getBackupMeta/getMediaTagsBulk）を追加。bin 構造を `cli/{main,db_client}.rs` に再編
- **import サブコマンド追加（TASK-68）**: TS CLI 廃止で失われる CLI import（JSON/CSV/TSV → bulkCreateMedia + tag + attribute）を Rust CLI に移植（`csv` crate 導入・readonly ガード・BOM 除去付き・Local/Remote 透過）

**ファイル:** `rust-sdk/src/bin/cli/{main,db_client}.rs`, `rust-sdk/Cargo.toml`, `rust-sdk/tests/cli_integration_test.rs`, `ts-sdk/src/cli.ts`（削除）, `ts-sdk/{package.json,src/{index,config,remote}.ts}`, `docs/design/decisions.md`

## Added

### 本番DB保護 — prod/stg 2ファイル体制・監査ログ・promote/discard（TASK-41,42,45,46,47,49,50,56,57,58,60）

LLM の無意識の prod 破壊を人間承認 gate なしで環境・構造により防ぐ prod/stg 2ファイル体制を実装（設計 `docs/design/db-protection.md`・TASK-41）。思想: 環境/構造で保護し人間 gate 不要（悪意はスコープ外）。3本柱（構造的保護 + 操作階層化(a)/(b) + システム gate）を P0〜P4 + promote/discard/復旧で実装。読込先切替・差分・observe・排他・(a)/(b)gate は個別エントリ（TASK-52/53/54/55/59）参照。

- **P0 基盤（TASK-42）**: prod/stg 2ファイル。`open_with_options` で WAL+busy_timeout 有効化（readonly 時は WAL スキップ）。`config.rs` に `Target`/`resolve_target`（優先順位: CLI `--target` > 環境変数 `KIJUKU_TARGET` > デフォルト **stg**）。prod 読込は readonly + no-migrate、migrate 前 snapshot 必須化。リモートへ `--target` 常時付与で prod RW 保護ホールを解消
- **pre-stash 常時化（TASK-47）**: `restore`/`create_pre_migrate_snapshot` の backup 有無ガードを廃止し常時退避。promote 用 `create_pre_promote_snapshot`（`-pre_promote.db`）を新設
- **copy_db_online（TASK-49）**: 任意 src→dst の Online Backup コピー API（src を READ_ONLY open・busy リトライ付き）。sync/promote の基盤
- **sync（TASK-50）**: prod(RO)→stg フル複製（`sync`/`sync-db` サブコマンド + stdin operation）。TS parity 修復も含む（`:memory:` DB での snapshot crash を `isFileBackedDb()` ガードで防止・`listBackups` が pre_* を除外せず一覧を汚染するバグ修正）
- **promote コア（TASK-56,57,58）**: `ProdRwScope`（RAII・`<prod>.lock` 排他 + pre-stash 強制・`ProdBusy`）で LLM に恒久 prod RW を持たせず操作内部でのみ prod RW を一時取得。observe gate 不合格なら `PromoteGateFailed` で拒否 → 既存 prod+WAL/SHM 削除 → `copy_db_online(stg→prod)`。TS SDK + CLI に parity（`acquireProdLock`/`promote`・readonly 拒否）
- **discard（TASK-45）**: stg 破棄・prod から再 sync・prod 不触。sync/discard 共通ヘルパ `run_prod_to_stg` で DRY 化。observe/promote/sync/discard の Rust `RemoteKijukuDB` parity が完成
- **prod 復旧（TASK-60）**: `list-pre-stashes`/`listPreStashes` で即時復旧経路（pre-stash 戻し）を一覧化。`listBackups` が除外する pre-stash（`*-pre_{migrate,restore,promote}.db`）を `backup/tmp/` から発見。短期/長期復旧は既存 restore、保持期間は `tmp_retention_secs`（既定7日）
- **監査ログ（TASK-46）**: observe/promote/sync/discard・(b)操作・diff 結果をサイドカー（prod DB 本体でなく `backup/meta/audit.log` JSONL）に記録し事後追跡可能に。`readAuditLogs`/`read_audit_logs`（from/to フィルタ付き）。運用 docs 更新（api.md・CLI README・DATABASE_SETUP.md に監査ログ・prod/stg 運用・`--target`/`KIJUKU_*` 環境変数）

**ファイル:** `rust-sdk/src/{lib,config,backup,prod_rw,error,remote,bin/cli}.rs`, `ts-sdk/src/{index,backup,remote,stg-session,config}.ts`, `rust-sdk/tests/{prod_rw_test,promote_test,sync_test,cli_integration_test}.rs`, `ts-sdk/test/integration/{sync,promote}.test.ts`, `docs/{api.md,DATABASE_SETUP.md,usage/cli/README.md,design/db-protection.md}`

### media root 配下のファイル操作（cp/mv/sync/trash）と安全枠（TASK-33,34,35,36,37,38,39）

LLM が `rsync --delete` 等で media root を全消去する不可逆事故を防ぐため、media root 配下のファイル操作を SDK 化し安全枠を提供。

- **media_root 設定 + パス安全化（TASK-34）**: `DBOptions`/`RemoteConfig` に `media_root` を追加（未設定時は file 操作 API を明示エラー）。root 外脱出拒否・symlink 解決後再検査・保護パス判定ヘルパ（`resolve_within_root`/`resolve_existing_within_root`/`is_protected`）
- **trash 論理削除（TASK-35）**: 削除/上書き対象を `media_root/.trash/<id>/<相対パス>` へ移動し物理削除を通常経路で不可に。`moveToTrash`/`restoreFromTrash`/`listTrash`/`purgeTrash`（purge のみ dry-run 付きの唯一の物理削除経路）。rsync ミラーリング相当も「余分ファイルを trash へ回す」ラッパで実現し生 `--delete` を触らせない
- **cp/mv/sync（TASK-36,33）**: `mediaCp`/`mediaMv`/`mediaSync`（safe モードのみ・dry-run ファースト・上書き/削除は trash 経由・DB 更新は optional フラグ）。`FileOpOptions`/`FileOpResult` 型
- **Remote 委譲 + 転送（TASK-37）**: `RemoteKijukuDB` で JSON-RPC 委譲 + `upload`/`download`（SFTP 直転送）。クライアント側で `resolve_within_root` + `is_protected` 検証
- **CLI（TASK-38,39）**: `file cp/mv/sync`・`trash move/list/restore/purge` サブコマンド + `--media-root` グローバルオプション（D1 バックエンドでは拒否）

**ファイル:** `rust-sdk/src/{media_path,trash,file_ops,lib,remote,bin/cli}.rs`, `ts-sdk/src/{media_path,trash,file_ops,index,remote}.ts`, `rust-sdk/tests/cli_integration_test.rs`, `docs/usage/cli/README.md`
...
- CLI: stdin `promote` 操作の params に `backupOpts` を追加。`backupOpts` は Rust CLI（stdin `promote`）と SDK API（TS Local/Remote）で受け渡し可能（TS CLI は TASK-65 で廃止されたため `--backup-opts` フラグは存在しない）
...
### backup を SDK 利用時にデフォルト有効化（TASK-23）

これまで CLI のみ有効・SDK では無効だったバックアップを、SDK レベルでデフォルト有効化。Rust は `DBOptions` を手動 `Default`（`backup: Some(BackupOptions::default())`）、TS は `backup` 未指定=有効/`null`=無効。

**ファイル:** `rust-sdk/src/{types,lib}.rs`, `ts-sdk/src/{types,index}.ts`

### mise run push で version の patch を自動 bump（TASK-32,72）

feature ブランチ push 時に version の patch を +1 して commit & push する `mise run push`（`scripts/dev/bump-and-push.sh`）を新設。デプロイバイナリの新旧を `--version` の数値だけで判別可能にする（dry-run・dev/main ブランチ拒否・version バリデーション・デタッチドHEAD ガード付き）。更新対象は Cargo.toml/package.json/README.md/cli/main.rs/version.ts/Cargo.lock の6箇所。CLI 整理（`cli.rs`→`cli/main.rs`）と TASK-69 での `version.ts` 追加に追従（TASK-72）。

**ファイル:** `scripts/dev/bump-and-push.sh`, `.mise.toml`
...
### 監査ログの堅牢化と記録漏れを修正（PR#58・TASK-78,79,80,81）

TASK-46 監査ログ機能（同一コミット）に対するレビュー指摘対応。

- **TASK-78**: TS SDK の `observe()`/`promote()` が監査ログに記録されなかった実装抜けを修正（`appendAuditToProd` 呼出を追加）
- **TASK-79**: `readAuditLogs()` が不正 JSONL 行で `JSON.parse` 例外になる問題を、行単位でパースエラーをキャッチしスキップするよう堅牢化（TS）
- **TASK-80**: `replicate_db()` で sync/discard 失敗時に監査ログが記録されなかった問題を、失敗時にも `AuditResult::Failure` を記録するよう修正（Rust）
- **TASK-81**: `read_audit_logs()` の from/to フィルタが ISO タイムスタンプの文字列比較だったのを、パースして正規化してから比較するよう修正（Rust）

**ファイル:** `ts-sdk/src/{index,backup}.ts`, `rust-sdk/src/{lib,backup}.rs`

### リモート SSH プール接続の one-shot プロセスでのハングを修正（PR#57・TASK-77）

プール接続（TASK-70/71）が `disconnect()` 未呼び出しの one-shot プロセスで TCP socket ref を保持し event loop を終了させない（ハングする）問題を修正。examples/04・README 例・E2E の afterAll で `remoteDb.disconnect()` を確実に呼ぶよう変更。README の誤記（未呼び出しでも閉じる）も修正。

**ファイル:** `examples/04-remote-operations.ts`, `docs/usage/sdk/ts/README.md`, `ts-sdk/test/e2e/sdk-remote.test.ts`

### backup メタデータ消失・パニック・無効値フォールバックを修正（PR#54・TASK-28,29,30,31）

- **TASK-28**: `readBackupMetaStore` が読込/パース失敗時に一律 `{}` を返し、後続の `updateBackupMeta` で既存メタデータが全消失する危険を、ファイル不在時のみ `{}`・失敗時は throw に修正（TS）
- **TASK-29**: `findBackupByIdInScope` の `parsed.extension` 直接アクセスを null 安全化（TS）
- **TASK-30**: 差分復元の `temp_path.parent().unwrap()` を `if let Some(parent)` に変更し、親ディレクトリ取得失敗時の本番パニックを回避（Rust）
- **TASK-31**: `parse_diff_detail` を `Result<DiffDetail,String>` に変更し、無効値（`--detail invalid` 等）をデフォルトフォールバックせずエラー表示して中断するよう変更。テスト5件追加（Rust）

**ファイル:** `ts-sdk/src/backup.ts`, `rust-sdk/src/{lib,bin/cli,diff}.rs`

### TS migrate v4/v5 再作成 DDL の原子性を保証（TASK-48）

TS migrate の v4/v5 再作成 DDL を `db.transaction()` で wrap し原子性を保証（PRAGMA foreign_keys は tx 外・version record は tx 内で Rust と同じ境界）。別トランザクションでの部分失敗の危険を解消。

**ファイル:** `ts-sdk/src/migration.ts`
