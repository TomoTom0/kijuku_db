...
- **監査ログ（TASK-46）**: observe/promote/sync/discard・(b)操作・diff 結果をサイドカー（prod DB 本体でなく `<prod>.audit.log` JSONL）に記録し事後追跡可能に。`readAuditLogs`/`read_audit_logs`（from/to フィルタ付き）。運用 docs 更新（api.md・CLI README・DATABASE_SETUP.md に監査ログ・prod/stg 運用・`--target`/`KIJUKU_*` 環境変数）
