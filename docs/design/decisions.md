...
## バックアップ復元判断支援（TASK-25,26,27）

復元先を判断する手段がなく実用性に欠けていた backup 復元を見直し、3機能を追加した。

### 決定事項

1. **差分バックアップ(.diff)の read-only 参照**: `with_backup_db` を拡張し、基底フルから一時フルDBを再構成して読み取り専用で開く。一時ファイルはコールバック終了後に `.db`/`-wal`/`-shm` ごと削除（WAL 副産物の残留を防ぐ）。
2. **`BackupSelector::ById` の追加**: タイムスタンプ文字列でバックアップを直接指定。`find_backup_by_id_in_scope`（基底フル `.db` のみ）とは別に、list 結果から `.diff` 含めて検索。
3. **差分表示（diff_with_backup）**: 現在DBとバックアップを5テーブル比較し added/removed/changed を算出。`added`=復元で復活、`removed`=復元で失われる、`changed`=復元で上書き。
4. **事後ラベル/メモはサイドカー方式**: ファイル名変更ではなく `backup/meta/backup-meta.json` に `{ entries: { <id>: { label?, note?, updatedAt } } }` で保持。

### サイドカー方式を採用した理由（ファイル名変更は不採用）

- **非破壊**: ファイル名を変えないため、外部ツール/スクリプト/ユーザがパスで参照している場合に壊れない。
- **自由文字列**: ファイル名規則（`[a-zA-Z0-9_-]`）に収まらない、改行含むメモ・UNICODE を扱える。
- **既存ラベル（作成時・ファイル名埋め込み）との両立**: サイドカー優先でマージし、`labelSource` で由来を区別。
- **間引き整合性**: `cleanup_old_backups` で実在しない id のエントリを掃除（数十行の追加で対応）。

### 副次修正

- `find_backup_by_id_in_scope` がスコープ引数を無視して `Auto` 固定を返すバグを修正（公開化に伴い発露）。
- 差分復元時の一時ファイル WAL 副産物（`-wal`/`-shm`）残留バグを修正。
